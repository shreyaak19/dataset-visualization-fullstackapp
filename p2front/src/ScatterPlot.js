 ...
 * Version: 1.0
 */
// The ScatterPlot provides an additional view on top of the BarChart view
...
// labels, values, their titles, and the circles for the scatterplot
const settings = {
    viewBox: {
        x: 0,
        y: 0,
...
// ScatterPlot function, takes in props the selectElement function from App.js,
// which is called using the event handler when the mouse is clicked on any
// of the circles, and it also takes in the selected state array as well as
  ...
  const handleSelected = (item, index) => {
    ...
    const paint = () => {
        ...
        svg
            ...
            .attr("x2", settings.values.x + settings.values.width + settings.circles.width - settings.lines.margin )
            .attr("y1", (item, index) => {
                return settings.labels.y - index * settings.circles.height / ((settings.values.max - settings.values.min) / settings.values.step);
            })
            .attr("y2", (item, index) => {
                return settings.labels.y - index * settings.circles.height / ((settings.values.max - settings.values.min) / settings.values.step);
            });
            ...
            svg
                ...
                .append("circle")
                .attr("cx", (item, index) => {
                    return settings.circles.x + (1 - settings.circles.ratio + index) * settings.circles.width / (dataset.data.length + 1 - settings.circles.ratio);
                })

                .attr("cy", (item, index) => {
                    return settings.labels.y - Object.values(item)[1] * settings.circles.height / (settings.values.max - settings.values.min);
                })
                .attr("r", 2)
        ...
        svg
            ...
            .append("text")
            .attr("x", (item , index) => {
                return settings.labels.x + (1 - settings.circles.ratio + index + settings.circles.ratio / 2) * settings.circles.width / (dataset.data.length + 1 - settings.circles.ratio);
            })
        ...
        svg
            ...
            .attr("y", (item, index) => {
                return settings.values.y + settings.values.height - settings.values.baseline - index * settings.circles.height / ((settings.values.max - settings.values.min) / settings.values.step);
            })
            .text((item, index) => {
